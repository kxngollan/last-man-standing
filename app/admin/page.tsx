          ...
          {data.rows.map((i) => (
            <li key={i.id} className={styles.issueRow} data-resolved={i.status === 'resolved'}>
              <span className={styles.issueChips}>
                <span className={`lms-chip ${i.status === 'open' ? 'lms-chip--out' : 'lms-chip--safe'}`}>
              ...
              <button className='lms-btn lms-btn--ghost lms-btn--sm' disabled={busy === i.id} onClick={() => toggle(i)}>
...
function FeedbackPanel() {
  ...
  return (
    <section className={`lms-panel ${styles.feedbackPanel}`}>
      ...
      <p className={styles.panelHint}>
        {data && data.count > 0
          ? `${data.count} ${data.count === 1 ? 'response' : 'responses'} · average ${data.averageRating ?? '—'}/5`
...
function PlayersPanel() {
  ...
  return (
    <section className={`lms-panel ${styles.users}`}>
      ...
      <p className={styles.panelHint}>
        Rename a player or verify their email by hand (e.g. when the confirmation email never arrived). Everyone sees
        players as first name + last initial.
      </p>
              ...
              <li key={u.id} className={styles.userRow}>
                <span className={styles.userNames}>
                  <input
                    ...
                    onChange={(e) => setEdits((m) => ({ ...m, [u.id]: { ...edit, firstName: e.target.value } }))}
                    disabled={rowBusy}
                  />
                  <input
                    className={`lms-field__control ${styles.userInput}`}
                    ...
                    value={edit.lastName}
                    onChange={(e) => setEdits((m) => ({ ...m, [u.id]: { ...edit, lastName: e.target.value } }))}
                    disabled={rowBusy}
                  />
                </span>
                ...
                <span className={styles.userChips}>
                  ...
                  <span className={`lms-chip ${u.emailVerified ? 'lms-chip--safe' : 'lms-chip--wild'}`}>
                    {u.emailVerified ? 'Verified' : 'Unverified'}
                  </span>
                  {!!u.referrals && (
                    <span className='lms-chip lms-chip--mono' title={`${u.referrals} confirmed referrals`}>
                ...
                <span className={styles.userActions}>
                  {dirty && (
                    <button
                      ...
                      disabled={rowBusy || !edit.firstName.trim() || !edit.lastName.trim()}
                      onClick={() =>
                        patch(u, {
...
export default function AdminPage() {
  ...
  async function action(key: string, url: string, payload?: unknown) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setResolveMsg(
          `Week ${body.gameWeek} is back in play — ${body.restored ?? 0} ${
            body.restored === 1 ? 'player' : 'players'
        ...
        setResolveMsg(
          body.total
            ? `Week ${body.gameWeek} synced — ${body.finished}/${body.total} fixtures final${
                body.playing ? `, ${body.playing} still to play` : ''
              }.`
            : `Week ${body.gameWeek} synced — no fixtures came back for that game week.`
        )
      }
      await load()
    } catch {
  ...
  const { current, pastGames, teamsSeeded, recovery } = data

  // Undo the last resolution. Sits in the current-game panel, and stays
  // reachable when the resolution finished the game (so there is no current
  // game at all) — that's the accident it exists for. Two clicks to fire.
  const undoButton = recovery && (
    <button
      ...
      onClick={() => {
        ...
        setUndoArmed(false)
        void action('unresolve', '/api/admin/unresolve', { gameId: recovery.gameId })
      }}
    >
      {undoArmed
        ? `Click again to confirm — Week ${recovery.gameWeek} results are wiped and everyone knocked out that week is back in`
        : `Undo Week ${recovery.gameWeek} resolve`}
    </button>
  )
        ...
        <div className={styles.grid}>
            ...
            {!current ? (
              ...
              <>
                ...
                <div className={styles.actions}>
                  ...
                  <button
                    className='lms-btn lms-btn--ghost'
                    disabled={busy !== null}
                    onClick={() => action('sync', '/api/admin/sync')}
                  >
              ...
              <>
                ...
                <div className={styles.actions}>
                  ...
                  {/* Fresh scores on demand, without waiting for the cron —
                      tells you whether the week is ready to resolve. */}
                  <button
                    className='lms-btn lms-btn--ghost'
                    disabled={busy !== null}
                    onClick={() => action('sync', '/api/admin/sync')}
          ...
          <section className={`lms-panel ${styles.panel}`}>
            ...
            >
              ...
              >
                {busy === 'create' ? 'Opening…' : current ? 'A game is already open' : 'Open registration'}
              </button>
            </form>
          </section>
              ...
              <ul className={styles.pastList}>
                {pastGames.map((g) => (
                  <li key={g.no} className={styles.pastRow}>
                    ...
                    <span className={`lms-chip ${g.tone === 'safe' ? 'lms-chip--safe' : 'lms-chip--out'}`}>
